(() => {
  ...
  const canvasIds = ['phaseCanvas', 'lineageCanvas'];
  ...
  function nudge(canvas) {
    ...
    requestAnimationFrame(() => {
      canvas.style.width = previous || '100%';
    });
  }

  function stabilize() {
    canvasIds.forEach(id => nudge(document.getElementById(id)));
  }

  window.addEventListener('load', () => setTimeout(stabilize, 0));
  window.addEventListener('orientationchange', () => setTimeout(stabilize, 240));

  ['resetPhaseLayout', 'resetLineageLayout'].forEach(id => {
